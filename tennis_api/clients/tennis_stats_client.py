...
class TennisStatsAPIClient(BaseAPIClient):
    ...
    async def get_player_stats(self, player_name: str) -> Dict:
        ...
        try:
            ...
            # Parse and combine all data into PlayerStats object
            player_stats = self._parse_detailed_player_stats(
                stats_response, surface_stats, recent_matches, player_name
            )
            
            # Convert PlayerStats to Dict for return
            return self._player_stats_to_dict(player_stats)
    ...
    async def get_player_stats_detailed(self, player_name: str, player_id: Optional[str] = None) -> PlayerStats:
        """
        Get comprehensive player statistics with PlayerStats return type
    ...
    async def get_rankings_with_date(self, tour: str = 'atp', date: Optional[str] = None) -> Dict:
        """
        Get current or historical rankings with date parameter
    ...
    def _parse_surface_stats(self, stats_data: Dict, surface: str) -> SurfaceStats:
        ...
        try:
            ...
            return SurfaceStats(
                ...
                serve_percentage=surface_data.get('serve_percentage', 0.6),
                return_percentage=surface_data.get('return_percentage', 0.3)
            )
    ...
    def _player_stats_to_dict(self, player_stats: PlayerStats) -> Dict:
        """Convert PlayerStats object to dictionary"""
        try:
            # Convert surface stats to dict
            surface_stats_dict = {}
            for surface, stats in player_stats.surface_stats.items():
                surface_stats_dict[surface] = {
                    'surface': stats.surface,
                    'matches_played': stats.matches_played,
                    'wins': stats.wins,
                    'losses': stats.losses,
                    'win_percentage': stats.calculated_win_percentage,
                    'serve_percentage': stats.serve_percentage,
                    'return_percentage': stats.return_percentage
                }
            
            return {
                'name': player_stats.name,
                'current_ranking': player_stats.current_ranking,
                'previous_ranking': player_stats.previous_ranking,
                'nationality': player_stats.nationality,
                'age': player_stats.age,
                'height_cm': player_stats.height_cm,
                'weight_kg': player_stats.weight_kg,
                'plays': player_stats.plays,
                'serve_stats': {
                    'first_serve_percentage': player_stats.serve_stats.first_serve_percentage,
                    'first_serve_win_percentage': player_stats.serve_stats.first_serve_win_percentage,
                    'second_serve_win_percentage': player_stats.serve_stats.second_serve_win_percentage,
                    'aces_per_match': player_stats.serve_stats.aces_per_match,
                    'double_faults_per_match': player_stats.serve_stats.double_faults_per_match,
                    'service_games_won_percentage': player_stats.serve_stats.service_games_won_percentage
                } if player_stats.serve_stats else {},
                'return_stats': {
                    'first_serve_return_points_won': player_stats.return_stats.first_serve_return_points_won,
                    'second_serve_return_points_won': player_stats.return_stats.second_serve_return_points_won,
                    'break_points_converted': player_stats.return_stats.break_points_converted,
                    'return_games_won_percentage': player_stats.return_stats.return_games_won_percentage,
                    'return_winners_per_match': player_stats.return_stats.return_winners_per_match
                } if player_stats.return_stats else {},
                'surface_stats': surface_stats_dict,
                'recent_matches': player_stats.recent_matches,
                'recent_form_factor': player_stats.recent_form_factor,
                'last_updated': player_stats.last_updated.isoformat() if player_stats.last_updated else None
            }
        except Exception as e:
            # Return basic dict if conversion fails
            return {
                'name': player_stats.name,
                'last_updated': player_stats.last_updated.isoformat() if player_stats.last_updated else None
            }
    ...
    def get_player_stats_sync(self, player_name: str, player_id: Optional[str] = None) -> PlayerStats:
        ...
        try:
            ...
            return PlayerStats(name=player_name, last_updated=datetime.now())
        except APIException:
            raise
        except Exception as e:
            raise APIException(f"Failed to get detailed stats for {player_name}: {e}")
